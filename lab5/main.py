...
from utils import random_divider
...
class Voter:
    ...
    def create_vote_message(self, candidate: int, pub_key: RSAKeys.public_key):
        div1, div2 = random_divider(candidate)
        encrypted_msg1 = rsa_encrypt(div1, pub_key)
        encrypted_msg2 = rsa_encrypt(div2, pub_key)
...
class EC:
    ...
    def register_vote(self, signature, encrypted_message, voter_id: UUID):
        ...
        try:
            ...
            return
        self.result_dict[voter_id] = verified_msg
...
class CentralEC:
    ...
    def calculate_votes(self, d1, d2):
        ...
        for voter_id, candidate in result.items():
            ...
            candidate = rsa_decrypt(candidate, self.rsa_keys.private_key)
...
def main():
    ...
    ec1 = EC(cec.get_anon_voters(), "ВК1")
    ec2 = EC(cec.get_anon_voters(), "ВК2")
    pub_key = cec.rsa_keys.public_key

    # v1
    s1, msg1, s2, msg2, v_id = v1.create_vote_message(8, pub_key)
    ec1.register_vote(s1, msg1, v_id)
    ec2.register_vote(s2, msg2, v_id)
    # v2
    s1, msg1, s2, msg2, v_id = v2.create_vote_message(9, pub_key)
    ec1.register_vote(s1, msg1, v_id)
    ec2.register_vote(s2, msg2, v_id)
    # v2 голосує повторно
    s1, msg1, s2, msg2, v_id = v2.create_vote_message(9, pub_key)
    ...
    s1, msg1, s2, msg2, v_id = v3.create_vote_message(12, pub_key)
    ec1.register_vote(s1, msg1, v_id)
    ec2.register_vote(s2, msg2, v_id)
    # v4 намагається проголосувати з невірним signature
    s1, msg1, s2, msg2, v_id = v4.create_vote_message(9, pub_key)
    ec1.register_vote(b"random", msg1, v_id)
    ec2.register_vote(s2, msg2, v_id)
    # v5 не зареєстрований
    s1, msg1, s2, msg2, v_id = v5.create_vote_message(9, pub_key)
    ec1.register_vote(s1, msg1, v_id)
    ec2.register_vote(s2, msg2, v_id)
