from rsa import RSAKeys, generate_rsa, rsa_sign, rsa_verify, rsa_decrypt, rsa_encrypt, rsa_blind, rsa_unblind
...
class Voter:
    ...
    def create_vote_message(self, signed_blinded_bulletin: list, cec_pub: RSAKeys.public_key):
...
def main():
    ...
    # voter 2
    bulletins, v_id, blind_factor = voter2.create_collection_of_bulletins(cec)
    random_signed_blinded_bulletin = cec.bulletins_proccessing(bulletins, v_id, blind_factor)
    msg = voter2.create_vote_message(random_signed_blinded_bulletin, cec_pub=cec.rsa_keys.public_key)
    cec.receive_vote_message(msg)

    # voter 5
    bulletins, v_id, blind_factor = voter5.create_collection_of_bulletins(cec)
    random_signed_blinded_bulletin = cec.bulletins_proccessing(bulletins, v_id, blind_factor)
    msg = voter5.create_vote_message(random_signed_blinded_bulletin, cec_pub=cec.rsa_keys.public_key)
    cec.receive_vote_message(msg)
